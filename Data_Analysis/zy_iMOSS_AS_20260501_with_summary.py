...
If you only need the summary for each file, not the plots, uncheck save-files to save time.
Rules:
...
from zy_importer_lite_V5 import FileSelectorWindow
...
class detect_immobility:
    def __init__(self):
        ...
        self.mobile_color = 'r'
        self.immobile_color = 'b'
        
        # --- NEW: Track if user closes the preview window ---
    ...
    def get_startTime_from_preview(self,df,start_time):
        ...
        def submit():
            try:
                self.start_time = trim_start_var.get()
                ...
                self.user_cancelled = False # Clear flag if successful
        ...
        def on_quit_button_click():
            print('User cancelled preview. Aborting batch.')
            self.user_cancelled = True # Flag so the main loop stops cleanly
            
            # --- SAFE EXIT SEQUENCE (NO MORE sys.exit!) ---
            root.quit()
            root.destroy()
        ...
        tk.Label(bottom_frame, text="Start_Time (s):",fg="red").grid(row=0, column=0, sticky='nsew')
        trim_start_var = tk.DoubleVar(root, value=start_time) 
        trim_start_entry = tk.Entry(bottom_frame, textvariable=trim_start_var, width=6)
        trim_start_entry.grid(row=0, column=1, sticky='nsew')
        trim_start_entry.bind("<KeyRelease>", update_vline)
        
        tk.Label(bottom_frame, text="fixed threshold:",fg="red").grid(row=1, column=0, sticky='nsew')
        threshold_factor_var = tk.DoubleVar(root, value=self.threshold)        
        tk.Entry(bottom_frame, textvariable=threshold_factor_var, width=6).grid(row=1, column=1, sticky='nsew')
        
        options = ['keep manual','keep auto','apply to all']
        select = options[self.auto_detect]
        selected_option = tk.StringVar(root, value=select)  
        tk.Label(bottom_frame, text="detection option:").grid(row=0, column=2, sticky='nsew')
        tk.OptionMenu(bottom_frame, selected_option, *options).grid(row=0, column=3, sticky='nsew')
        
        save_files_var = tk.BooleanVar(root, value=self.save_files)
        tk.Checkbutton(bottom_frame, text="save files", variable=save_files_var,fg='red').grid(row=1, column=2, sticky='nsew')  
        
        save_pdf_var = tk.BooleanVar(root, value=self.save_PDF)
        tk.Checkbutton(bottom_frame, text="save fig as PDF", variable=save_pdf_var,fg='red').grid(row=1, column=3, sticky='nsew') 
        
        debug_var = tk.BooleanVar(root, value=self.debug)
        tk.Checkbutton(bottom_frame, text="debug?", variable=debug_var,fg='red').grid(row=2, column=2, sticky='nsew') 
        
        tk.Button(bottom_frame, text="Submit", command=submit).grid(row=2, column=3, sticky='se')
        
        # plt.close(fig) #'all'
        # Layout weights
        for i in range(4): bottom_frame.columnconfigure(i, weight=1)
        
        root.protocol("WM_DELETE_WINDOW", on_quit_button_click)
        root.mainloop()

        # --- FIX: Close the pyplot figure AFTER the window closes to prevent memory leaks ---
        # plt.close(fig) 
    ...
    def detailed_extract_signal(self,df):
        ...
        period_threshold = flat_signal.mean()*0.9 + flat_signal.min()*0.1 
        self.period_threshold = period_threshold
        print(f'period_threshold={period_threshold}')
        
        def find_baseline(df,nPoints=800): 
            df_middle = df.iloc[len(df) // 3:-len(df) // 3]  
        ...
        if self.auto_detect ==1:
            try: 
                continue_signal_check = int(270/self.time_step) 
                flat_mask = (flat_signal>period_threshold)[::-1].rolling(window=continue_signal_check).sum()[::-1] == continue_signal_check 
            ...
            df_trim = df.loc[period_separating_index:]        
            ...
            if self.preview_startTime:
                self.get_startTime_from_preview(df,start_time)
            else:
                self.start_time = start_time
                
        elif self.auto_detect ==2:
            print(f'reuse start_time to all at: {self.start_time}')
            
        elif (not self.auto_detect):  
            self.get_startTime_from_preview(df,start_time)
            
        # --- ABORT CHECK: If user hit 'X' or Quit on the preview window ---
        ...
        baseline = find_baseline(df_extracted,nPoints=self.BL_points) 
        ...
        elif self.save_files: #just plot the signal/filtered signal around start_time 
    ...
    def plot_detection(self,df,used_signal_col):
        ...
        self.total_mobile_time = round(total_mobile_time,3)
        print(f'total mobility time = {self.total_mobile_time} s')
        self.total_immobile_time = round(total_immobile_time,3)
        print(f'total immobility time = {self.total_immobile_time} s')
        
        self.last4_immobile_time = round(df[used_immobile_col].loc[df[time_column] >=120].value_counts().get(1, 0) * time_step,3)  
        print(f'Last 4min immobility time = {self.last4_immobile_time} s')
        ...
        if self.save_files or self.debug:
    ...
    def run(self,file):
        ...
        self.filename = self.filename_refill(file)
        ...
        if 'systemTime' in df_loadcell.columns: 
        ...
        df_raw['Signal'] = df_raw['Signal'] / 1000 
        self.time_step = df_raw[time_column].diff().mean().item()
        
        # Extract signal
        df = self.detailed_extract_signal(df_raw) 
        
        # --- ABORT CHECK: Safely exit this run if user cancelled the preview ---
        if df is None:
            return False
        ...
        df['Cumulative_Immobility'] = df['Immobile'].cumsum()*self.time_step
        ...
        df['Cumulative_Immobility_filter'] = df['Immobile_filter'].cumsum()*self.time_step
...
if __name__ == "__main__":
    ...
    if file_selector.user_cancelled:
        ...
        ML_summary = pd.DataFrame(columns=["Mouse_ID", "Total_immobility_time", "last4_immobile_time", "Start_time", "Latency"])
        
        start_time = time.time()
        
        for file in list(selected_files):
            success = TST.run(file)   
            
            # --- BATCH ABORT: Stop loop if user cancelled the preview window ---
            if not success:
                print("\n🚨 User cancelled preview! Stopping remaining files.")
                break 
            
            ML_summary.loc[len(ML_summary)] = [
                TST.filename,
                round(TST.total_immobile_time, 3),
                round(TST.last4_immobile_time,3),
                round(TST.start_time, 3),
                round(TST.latency, 3)
            ]
        ...
        # It still saves the CSV for the files it successfully processed before the cancel!
        if not ML_summary.empty:
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            csv_name = f"all_in_one_threshold_{TST.threshold}_{timestamp}.csv"
            csv_path = os.path.join(TST.output_path, csv_name)
            os.makedirs(TST.output_path, exist_ok=True)
            ML_summary.to_csv(csv_path, index=False)
