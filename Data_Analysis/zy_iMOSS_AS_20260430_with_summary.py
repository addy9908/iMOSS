...
if __name__ == "__main__":
    ...
    # 20260430: output summary
    ML_summary = pd.DataFrame(columns=["Mouse_ID", "Total_immobility_time", "Start_time", "Latency"])
    ...
    for file in list(selected_files):
        ...
        ML_summary.loc[len(ML_summary)] = [
            TST.filename,
            round(TST.total_immobile_time, 2),
            round(TST.start_time, 2),
            round(TST.latency, 2)
        ]
    ...
    # save summary in one file when batch analysis
    # Create timestamp string
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Build filename with threshold + timestamp
